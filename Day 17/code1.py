...
def step(dim):
    dim = np.pad(dim, ((1, 1), (1, 1), (1, 1)), 'constant')
    nextDim = np.copy(dim)
    adjDim = np.copy(dim)
    z = len(dim)
    y = len(dim[0])
    x = len(dim[0][0])
    for (idz, idy, idx), cube in np.ndenumerate(dim):
        adjacent = sum(dim[
            max(idz - 1, 0):min(idz + 2, z),
            max(idy - 1, 0):min(idy + 2, y),
            max(idx - 1, 0):min(idx + 2, x)].flatten())
        adjDim[idz][idy][idx] = adjacent
        if cube:
            if not (3 <= adjacent <= 4):
                nextDim[idz][idy][idx] = 0
        else:
            if adjacent == 3:
                nextDim[idz][idy][idx] = 1
    return nextDim
